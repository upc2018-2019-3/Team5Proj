	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("adminlogin"))
		{
			...
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				...
				String sql1="";
				if (utype.equals("超级管理员"))
				{
				 sql1 = "select * from allusers where username='"+username+"' and pwd='"+password+"'  "; 
				 }

				  if (utype.equals("院系学生"))
				{
					sql1 = "select * from xueshengxinxi where yonghuming='"+username+"' and mima='"+password+"' and issh='是'"; 
				}
			...
			if(userlist1.size()==1)
			{
				...
				{
				request.getSession(). setAttribute("username", userlist1.get(0).get("yonghuming"));  
				request.getSession(). setAttribute("zy",  userlist1.get(0).get("zhuanyemingcheng"));  
				request.getSession(). setAttribute("cx",utype );  
				}
